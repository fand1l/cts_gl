...
### The look: Material 3, as far as it reaches

`material.py` holds the whole of it — the palette, the colour roles, the type
scale, the shape scale, the 8 dp grid and the motion easings — so that what used
to be a decision in ten different files is one table read from one place. The
rules are the specification's, at <https://m3.material.io/>. The reason for
following them is not fashion: the gesture this program imitates is Google's own
*Circle to Search*, which on the phone it came from is drawn in MD3.

**What crosses over** is the platform-neutral part: the roles, and the rule that
colours only ever pair as `X` and `on-X`; the type scale; the corners; elevation
as tone; the easings, which are plain cubic-béziers; the spacing grid. **What
does not** is Roboto, the `dp` table, spring physics, and every line of Compose —
the MD3 reference is Compose-first and this is PyQt6.

Three places where MD3 and this program disagreed, and how each was settled:

* **Dynamic colour was already done, differently.** MD3 grows a scheme from the
  wallpaper. The accent here is the Qt palette's Highlight — the KDE accent
  chosen in System Settings — which is the same idea answered better for this
  desktop, because it agrees with the rest of it. So: the MD3 roles, on the KDE
  seed.
* **Tonal elevation needs a surface and there was not one.** Everything the
  overlay draws sits on a frozen photograph of somebody else's screen. So the
  *dimming* became the surface: no longer flat black but the dark scheme's
  `surface`, a near-black carrying the accent's hue, with the action bar at
  `surface-container-high` and the readouts at `-highest` above it. Depth is
  something to read off the colours rather than a shadow this program never drew.
* **Roboto would have been wrong.** A KDE application that overrides the font set
  in System Settings is a badly behaved KDE application. Every role is a ratio
  against MD3's own `body-medium`, applied to whatever `QFont` the desktop gives.

Two things worth knowing before changing any of it:

* **The overlay has two backgrounds and only one of them is ours.** Things above
  the dimming take the dark scheme. The handles, the move grip, the selection
  outline and the loupe's crosshair are drawn on the part deliberately left
  *un*-dimmed — somebody's own window, most often a white one — and take the
  light scheme, whose `primary` is a saturated mid-tone rather than tone 80.
* **MD3 does not replace the two-tone edges.** Its containers sit on its own
  surfaces, where a change of tone separates them; these sit on a stranger's
  desktop, which may be any colour. Every container drawn over the frozen screen
  keeps a dark hairline outside a light one (`_draw_two_tone`). The two systems
  answer different questions.

**The tones are computed in CIELAB rather than HCT.** HCT's *tone* is exactly
CIE L\*, so the tone axis — and every contrast the role pairs promise — is exact;
only hue and chroma differ slightly from CAM16's, most visibly on saturated
blues. `tests/test_logic.py` checks the property rather than a table of bytes:
every `X`/`on-X` pair, for every seed tried and in both schemes, at 4.5:1 or
better. One deliberate departure: a seed with no chroma reports a hue made of
rounding noise, and MD3's core palette would force 48 chroma onto it — so a grey
theme would come out green. It gets Breeze's hue instead.

The pictures in this documentation are regenerated by `tools/make-screenshots.py`
from the real widgets, in both languages, so they cannot drift away from it.
