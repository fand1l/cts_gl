...
| | |
|---|---|
| done | **2 search the text** · **8 what will be sent** · **9 the same area** · **1 redact** · **7 colour** · **5 pin** · **6 history window** · **4 `--doctor`** · **11 try another way** · **13 no-mouse** · **12 QR** · **10 drag out** — by the other route, see below · **15 Material 3** |
| dropped | 3 delay · 14 annotations |

**All fifteen are done.**  15 was last and on its own, as it said it would be,
and it repainted what the other fourteen left behind.
...
## 15. A new design, on Material 3 — **done**
...
*What shipped:* `material.py` — the palette, the roles, the type scale, the
shape scale, the spacing grid and the motion easings, in one place, so that the
ten rows of the table above stopped being ten separate decisions.  All three
disagreements were answered the way they were framed, and the answers held.

* **The KDE seed, the MD3 roles.**  Exactly as written.
* **The dimming became the surface**, which was the "more interesting and more
  work" option, and it turned out to be the only reading under which tonal
  elevation means anything here: the dim is the dark scheme's `surface` — a
  near-black carrying the accent's own hue — and the bar and the readouts above
  it are `surface-container-high` and `-highest`, lighter tones of that same
  neutral palette.  Depth is now something to *read off* the colours instead of
  a shadow this program never drew.
* **The scale, not the typeface.**  Every role is a ratio against MD3's own
  `body-medium`, applied to whatever `QFont` System Settings hands over.

Four things came out of building it that the sketch had not asked:

* **The overlay has two backgrounds and only one of them is ours.**  This is the
  correction the plan most needed.  Everything above the dimming is on a surface
  we control, so it takes the dark scheme — but the handles, the move grip, the
  selection outline and the loupe's crosshair are drawn on the part deliberately
  left *un*-dimmed, which is somebody's own window and most often a white one.
  A dark scheme's `primary` is tone 80 because it expects darkness beneath it,
  and on a white document it is barely there.  So there are two schemes from the
  one seed, and the things that touch the content take the light one.  Found by
  looking at the first regenerated screenshot, where the handles had gone pale.
* **MD3 does not replace the two-tone edges, and never could.**  Its containers
  sit on its own surfaces, so a change of tone separates them; these sit on a
  photograph of a stranger's desktop.  Tone is depth against a *known*
  background and nothing at all against an unknown one.  So every container
  drawn over the frozen screen keeps a dark hairline outside a light one, and
  the two systems answer different questions rather than one overruling the
  other.
* **The tones are built in CIELAB, not HCT.**  MD3's own space is Google's, and
  its *tone* is exactly CIE L\* — so the tone axis, and every contrast the
  `X`/`on-X` pairs promise, is exact; only the hue and chroma of a generated
  shade differ slightly from CAM16's.  Three hundred lines of colour appearance
  model to move a few shades by an amount nobody can name was not the trade.
  The tests check the property the specification is really promising — that
  every pair is readable — and it holds at 4.5:1 or better for every seed tried,
  in both schemes.  One deliberate departure beyond that: a seed with no colour
  in it reports a hue made of rounding noise, and MD3's core palette would force
  48 chroma onto it, so a grey theme would come out green.  It gets Breeze's hue
  instead of a coin toss.
* **A Qt trap, uncovered by the type scale.**  The hints under the settings are
  wrapped prose, and a wrapped `QLabel` reports a minimum height of one line —
  it is entitled to assume it might be given unlimited width.  The settings
  window therefore opened *shorter than its own contents* and laid the sentences
  over the controls below them; `docs/POLISH.md` item 5 recorded working around
  it ("so it only ever grows").  Moving the hints to `body-small` made them wrap
  sooner and turned that squeeze into a visible overlap, which is how it was
  finally traced.  `SupportingText` answers `heightForWidth` properly, which
  fixed the overlap and made the window taller than a laptop screen — so the
  tabs scroll now.  Both halves are the same fix.

The stroke is the one row of the inventory that did **not** change, and that is
the finding rather than an omission: `docs/STROKE.md` derived it from two
photographs of Circle to Search running on a phone, which is MD3 as Google
actually draws this exact gesture.  Redrawing it from the spec's components
would have moved it *away* from the thing being imitated.
