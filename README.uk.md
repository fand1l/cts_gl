...
* **Намальовано за Material 3**, у вашому акцентному кольорі. Жест, який це
  наслідує, — Google-івський, і на телефоні, звідки він походить, його малюють
  саме в MD3. Тож ролі кольорів, шкала шрифту й форми — зі специфікації, але
  виростають з акценту, вибраного вами в системних параметрах, і з вашого ж
  шрифту.
...
* **Або приколоти до екрана** й лишити там, поки працюєте, — коли те, що треба
  прочитати, за вікном, у яке треба друкувати. *Ctrl* і тягнути — і кроп
  перелітає з приколки просто в інше вікно.
...
* **QR-код читається тут, а не надсилається.** Обведіть його — і панель спершу
  запропонує відкрити посилання з нього, а не картинку його в Google.
  Розшифровується на вашій машині, десь за мілісекунду.
...
| **Стрілки** | навести без миші; *Пробіл* — один кут, потім другий |
...
**Зовсім без миші:** стрілки піднімають приціл і рухають його — *Ctrl*, щоб
їхати, *Shift*, щоб по пікселю. *Пробіл* пришпилює один кут, стрілки задають
від нього розмір, *Пробіл* удруге бере виділення. Далі це те саме виділення,
що чекає, з тими самими клавішами. *Esc* відпускає кут, не відпускаючи знімок.
...
| **Відкрити посилання** | *Enter* | коли у виділенні є QR-код |
...
А **з натиснутим *Ctrl* приколку можна перетягнути** — і кроп впаде просто в
інше вікно: у чат, у документ, у графічний редактор. Звичайне тягнення пересуває
саму приколку, бо у вікна без заголовка заголовком є воно все, тож кропові
потрібне власне натискання; поки тримаєте *Ctrl*, курсор про це й каже. Клавіші
для цього немає — кинути можна лише кудись, а клавіатурі нікуди показати.
...
**Не вдалося надіслати.** У сповіщенні тепер є кнопка **Спробувати інакше**.
Надіслати картинку можна двома способами — її публікує ваш браузер або сама
програма, — і ламаються вони з різних причин, тож кнопка міняє спосіб, а не
повторює той самий. Виділення нікуди не зникло. Пропонується один раз, далі
просто повідомляє про помилку.
