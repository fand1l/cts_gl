...
```bash
./install.sh update               # git pull і перевстановлення — налаштування лишаються
...
Запамʼятати варто `update`. `reinstall` не має до git жодного стосунку — він
ставить те, що лежить у теці, у якому б вона стані не була; `update` спершу
тягне, і робить це в порядку, який не може зробити гірше: якщо pull не вдався,
нічого не видалено, і працює та сама копія, що й працювала.
