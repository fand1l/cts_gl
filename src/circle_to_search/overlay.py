...
class SelectionOverlay(QWidget):
    ...
    def _draw_chips(self, painter: QPainter) -> None:
        ...
        painter.setFont(font)
        painter.setRenderHint(QPainter.RenderHint.Antialiasing, True)
        # On the scheme, like everything else drawn on the dimming.  A chip is
        # the one offer on an untouched screen, so it takes the container role
        # rather than a neutral one — and its own two-tone edge, because it sits
        # on a photograph of somebody else's desktop rather than on a surface of
        # ours, which is the rule MD3 never had to have.
        for chip in self._chips:
            pill = QRectF(chip.pill)
            radius = pill.height() / 2
            painter.setPen(Qt.PenStyle.NoPen)
            painter.setBrush(self._scheme.primary_container)
            painter.drawRoundedRect(pill, radius, radius)
            self._draw_two_tone(painter, pill, radius, self._scheme.outline_variant)
            painter.setPen(QPen(self._scheme.on_primary_container))
            painter.drawText(chip.pill, int(Qt.AlignmentFlag.AlignCenter), chip.label)
